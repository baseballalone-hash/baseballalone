...
- **능력치 영향도 (2026-05-29 감사):** 10개 능력치 중 **8개**(contact/power/eye/speed/defense · velocity/control/breaking)는 타석 결과에 직접 작용하나, **멘탈·투수 스태미나(능력치)는 at-bat 결과에 직접 영향 없음** — 컨디션 회복 + OVR 집계(종합점수·대표팀 선발·등판확률)에만 쓰임. → 멘탈에 실제 경기 역할을 줄지 등 설계 결정 대기. 상세·후보안: `.claude/handoff.md` "미해결 버그/설계 이슈" 참고
- **BGM 안드로이드 백그라운드 미정지:** 모바일에서 앱을 백그라운드로 보내도 `<audio>` 재생이 계속됨 (`visibilitychange`/`pagehide` pause 미구현). 수정안 확정, 착수 대기 (`handoff.md` 이슈 A)
