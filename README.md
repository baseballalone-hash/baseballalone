...
- 졸업 후 게임 종료 분기 (`career.js:transitionToStage("retire")`)는 있지만 회귀(NewGame+)는 미구현 — EVENTS.md §3.5 참고
- NPC 풀은 학년 진급 시 새로 생성 (시즌 사이 NPC 자체 성장/은퇴 미구현)
- 부상 회복은 주 단위 자동 (가속/치료 액션 없음). 토미존 후 재활 미니이벤트 X
- 경기는 결과만 시뮬 (POV 시각화는 결승/PO/시즌 라이브 모달에만 — Phase 2 본격 시각화 예정)
- 타순/상황별 결과 단순화 (Phase 2에서 상세화)
- 일반 시즌 경기는 라이브 모달 없음 — 결승/PO/올스타/국제대회만 라이브
- 컨디션 슬럼프 사이클은 능력치에만 영향, 이벤트 트리거 미연결
...
│   │   ├── simulator.js     # 경기 자동 시뮬레이션 (매치업·코치판단·5선발 로테이션·walkoff)
│   │   ├── week.js          # 주 단위 진행
│   │   ├── career.js        # 진로 분기 + FA + 트레이드
│   │   ├── autoTrain.js     # 자동훈련 프리셋
│   │   ├── awards.js        # 시즌 수상 (MVP/타격왕/홈런왕/골든글러브 등)
│   │   ├── finals.js        # HS 토너먼트 결승 모달
│   │   ├── postseason.js    # KBO/MLB 포스트시즌 + 시리즈제
│   │   ├── seasonEvents.js  # 올스타·WBC·올림픽·아시안게임·프리미어12 라이브 모달
│   │   ├── military.js      # 군 입대 (상무/경찰/사회복무)
│   │   ├── offseason.js     # 휴식기 5 카테고리 + 20+ 이벤트
│   │   ├── milestones.js    # 단경기·통산 마일스톤 자동 검출
│   │   ├── hallOfFame.js    # 은퇴 시 헌액 점수 + 등급
...
| 영역 | 상태 |
|---|---|
| **회귀 시스템 (NewGame+)** | v0.5.3 설계 확정, 구현 대기 — `EVENTS.md §3.5` (5탭 상점, HoF 점수 재활용, 영구/재구매 하이브리드) |
| **세이브 마이그레이션 보강** | v0.5 신규 필드(`contract`, PO `tournamentHistory`, `milestones`) 폴백 |
| **컨디션 슬럼프 → 이벤트** | 장기 저조 시 회복 미니이벤트 트리거 |
| **Phase 2 본격** | POV 시각화 + 구종/상성/타순 (현재 결승/PO/시즌이벤트만 라이브) |
...
- [ ] 회귀 시스템 (NewGame+) — v0.5.3 설계 확정, 구현 대기 (EVENTS.md §3.5)
