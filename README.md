...
## 📖 문서 안내 (처음 보는 사람 / 다음 세션용)

이 저장소는 **문서 주도(doc-driven)** 로 개발한다. 구현자(Claude Code)는 세션마다 바뀌고 이전 대화 기억이
없으므로, **저장소의 문서가 곧 "공유된 뇌"** 다. 아래 문서들만 읽으면 코드 없이도 전체 상태를 파악할 수 있다.

| 문서 | 역할 (무엇을 담나) | 언제 읽나 |
|---|---|---|
| **`README.md`** (이 문서) | 게임 소개 · 기술 결정 · 실행법 · **버전별 changelog** · 핵심 시스템 · 파일 구조 · 로드맵 | 전체 그림을 잡을 때 |
| **`.claude/handoff.md`** | **현재 진행 상황 · 결정 사항 · 다음 할 일 · 미해결 버그 · 백로그** | **세션 시작 시 가장 먼저** |
| **`DEVELOPMENT.md`** | 개발 방식(왜 문서를 쓰나) · 도구/의존성 · **테스트·검증 방법** · 배포 · **에셋 파이프라인** | 직접 개발/검증/배포할 때 |
...
| **`assets/README.md`** | 이미지/오디오 **파일명·스펙 규약** + 생성 프롬프트 시트 (완료/TODO 구분) | 에셋 생성·추가할 때 |
| 메모리(`~/.claude/.../memory/`) | 세션 간 유지할 **사용자 선호·작업 규칙** (저장소 밖, 자동 로드) | (자동) |

**처음이라면 읽는 순서:** `handoff.md`(지금 상태) → 이 `README`(전체 그림) → 필요 시 `DEVELOPMENT`/`EVENTS`.
원칙: **코드 = 무엇 / 문서 = 왜·상태·다음.** 모든 커밋/푸시에 관련 문서 갱신을 포함한다.
...
| 영역 | 작업 |
|---|---|
| **fix(배포)** | `firebase.json` hosting ignore 에 `assets/**` 가 있어 **이미지·BGM 이 배포 제외**되던 치명 버그 → md 만 제외로 변경. 라이브에서 타이틀 이미지/BGM 정상화. `gemini_images/`·`aibgm/` 원본은 ignore. |
...
캐릭터 생성 미리보기와 결승 POV 씬을 Gemini 일러스트로 교체. 에셋 없으면 기존 SVG 폴백(무손상).
