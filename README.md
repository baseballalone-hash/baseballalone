...
이 프로젝트는 안정적인 유지보수와 기능 확장을 위해 문서화를 철저히 관리하고 있습니다. 아래 문서를 통해 코드 구조와 시스템 설계를 한눈에 파악할 수 있습니다.

| 문서 | 역할 (무엇을 담나) | 언제 읽나 |
|---|---|---|
| **`README.md`** (이 문서) | 게임 소개 · 기술 결정 · 실행법 · **버전별 changelog** · 핵심 시스템 · 파일 구조 · 로드맵 | 전체 그림을 잡을 때 |
| **`DEVELOPMENT.md`** | 개발 환경 · 도구/의존성 · **테스트·검증 방법** · 배포 · **에셋 파이프라인** | 직접 개발/검증/배포할 때 |
| **`EVENTS.md`** | 실제 야구 대비 **이벤트 갭 분석** · 구현 노트 · probe 사용법 · 변경 로그 | 야구 컨텐츠/이벤트/밸런스 작업 |
| **`MONETIZATION.md`** | **광고(GameMonetize SDK) 설정** · 채울 gameId · 노출 시점 · 정책 | 수익화/광고 작업 |
| **`assets/README.md`** | 이미지/오디오 **파일명·스펙 규약** 및 리소스 목록 | 에셋 생성·추가할 때 |

원칙: **코드 = 무엇 / 문서 = 왜·상태·다음.** 모든 커밋/푸시에 관련 문서 갱신을 포함합니다.
...
| 그래픽 | **SVG (코드 생성)** | 리소스 의존성을 줄이고 벡터 형태로 동적 그래픽 구현 |
...
외부 의존성 zero. 모든 핵심 시각 요소는 SVG 코드 생성을 기반으로 하며, 에셋 추가가 유연합니다.
...
### v0.7 추가 ✓ — 경기 디테일 · 밸런스 대개편 · 회귀/상점 · UX

| 영역 | 작업 |
|---|---|
| **불펜/마무리/대타/대주자** (`simulator.js` + `npc.js`) | `assignBullpenRoles` — SP 5명 보장 + RP 중 OVR순 마무리(CL)/셋업(SU)/중간계투(MR). 세이브 상황 9회 마무리·8회 셋업 우선. **대타(PH)/대주자(PR)** — 후반 접전에 약한 타순을 벤치 최고타자로, 느린 주자를 빠른 벤치 주자로 교체. 메인 구원 등판(pitchRole RP) 마운드 주입. |
| **능력치 미달 시 교체출장** | `buildLineup` 을 **팀 내 상대평가**로 — 건강 야수 OVR 상위 9명만 선발, 메인도 9위 밖이면 벤치 → 후반 대타로 참여. 투수도 선발 탈락 시 낮은 확률로 구원. 반환에 `batStatus`/`pitchStatus`. |
| **훈련 효율 너프** (`player.js`) | 글로벌 계수 `TRAIN_GAIN_COEFF` 0.8 → **0.25**. 첫 인생 고1은 한 종목만 파도 OVR ~55-65 (선발/대회 주전 진입 불가). 22시즌 피크는 cap 수렴이라 유지. |
| **자동훈련 = 스탯별 목표 비중** (`autoTrain.js`) | 종목 가중치 → **스탯별 목표 비중(0~1, 목표치=비중×cap)**. 목표 대비 부족분 비례로 육성, 목표 도달 시 중단(멘탈 폭주도 자연 차단). 슬러거=파워 100/타격형, 파이어볼러=구속 100/투수형 등 비중대로 빌드. 누수 스탯 감점. |
| **대표팀/올스타 선발 = 역할기반 rating** (`seasonEvents.js` + `offseason.js`) | 누적 fame 게이트 폐지(올스타만 fame+실력 병행). `nationalTeamRating` = max(타OVR,투OVR)+min×0.25 — 특화/양방향 공정 평가. 임계: 올스타 120·AG 135·P12 148·WBC 158·올림픽 165·U-18 90. |
| **드래프트/콜업/MLB 진입 역할기반** (`career.js`) | flat 평균 → `nationalTeamRating`. 신인은 거의 2군 시작(특출 즉시전력만 1군 직행), 성장 시 콜업. 콜업 임계 pro2→1군 113 / mlb_a→aa 128 / aa→aaa 145 / aaa→메이저 165. MLB 오퍼 풀 크기 비례 버그 수정. |
| **노쇠 강등 + 은퇴 선택** (`career.js` + `weekly.js`) | `checkDemotion` — rating 이 유지 임계 미달 시 한 단계 강등(1군<95 등). **핑퐁 방지**: 유지 임계 < 콜업 임계. 강등 시 [계속]/[은퇴] 모달. |
| **상점 캡 = 스탯별 +5** (`shopCatalog.js` + `regression.js` + `player.js`) | 그룹 tier → **스탯 10종 각각 캡 +5**(전 stage 공통, 상한 없음, 가격 점증). `getPlayerStatCap(player, stat)` 스탯별. 레이다/막대 그래프 max 동적(stage base + 스탯별 보너스). |
| **유물 레벨업** (`shopCatalog.js` + `traitEffects.js`) | 유물 가격·효과 하향 후 **재구매 시 레벨업**(효과·가격 점증). 예: past_life_notes ×2.0 → Lv.1 ×1.3/레벨당 +0.25. `relicLevels` 모델 + 캐릭터 전달. |
| **MLB 30개 구단** (`teams.js`/`teams.en.js`) | 6팀 → AL/NL 6디비전 전체 30구단, 현실적 strength(70~88). |
| **데이터 전체 초기화** (`state.js` + `cloudSave.js` + `menu.js`) | `resetAllData` — ninthinning.* localStorage 전부 제거(locale 유지). 로그인 시 Firestore 문서도 삭제(`deleteFromCloud`). 확인 모달 경유. |
| **시작 화면 분리 + 1페이지 생성창** (`menu.js` + `main.js`) | 타이틀/로딩 화면 신설(로고+스피너 → 이어하기/새 게임 + 로그인·클라우드·상점·데이터초기화). 캐릭터 생성창은 생성 전용 + 스마트폰 360×640 한 화면. 이름 최초 진입 시 언어별 평범한 이름 랜덤 prefill. |
| **fix(weekly)** | 투수교체 로그 "강판/등판" 명시, 이벤트 없는 공수 연속 시 구분선(`-----`) 중복 제거. |

### v0.7.1 추가 ✓ — 에셋 배포/오디오 + 모달·콜업 fix

| 영역 | 작업 |
|---|---|
| **fix(배포)** | `firebase.json` hosting ignore 에 `assets/**` 가 있어 **이미지·BGM 이 배포 제외**되던 치명 버그 → md 만 제외로 변경. 라이브에서 타이틀 이미지/BGM 정상화. 원본 에셋 소스는 ignore. |
| **fix(오디오)** | 첫 제스처 때 AudioContext suspended 라 효과음이 안 나던 문제 → suspended 면 resume 후 재생. **모든 버튼 클릭에 효과음**(main.js 위임 리스너). |
| **fix(모달 통과클릭)** | 보상/결과 모달을 pointerdown 으로 닫으면 같은 탭의 click 이 뒤 요소(훈련 버튼 등)에 떨어져 오작동 → 전역 가드: 모달 안에서 시작된 탭의 click 이 모달 밖이면 차단. |
| **fix(콜업)** | KBO 2군→1군 콜업 시 다른 구단(예 광주→서울)으로 바뀌던 버그 → 같은 팀 풀이면 **구단 유지**(`teamForStageKeeping`). 콜업이 로그만이라 모르고 지나가던 것 → **콜업 모달** 추가(강등 모달과 대칭). |
| **fix(i18n)** | 프로/MLB 휴식기 타이틀이 "휴식기 — 4학년 종료" 처럼 학년 표기되던 것 → 프로는 "만N세 시즌 종료"(`offseason.titlePro`). |

### v0.7.2 추가 ✓ — BGM 백그라운드 정지 (이슈 A)

| 영역 | 작업 |
|---|---|
| **fix(오디오)** | 모바일에서 앱을 백그라운드로 보내도 `<audio>` BGM 이 계속 재생되던 버그 → `initAudioUnlock()` 에 `visibilitychange`/`pagehide`(→pause), `pageshow`/visible(→play) 핸들러 추가. `currentBgm` 은 유지해 복귀 시 멈춘 지점부터 이어 재생(음소거·unlock 상태 존중). **실기 검증 대기**(샌드박스에선 audio 백그라운드 거동 재현 불가). |

### v0.7.3 추가 ✓ — 멘탈·스태미나 능력치 실질화 (이슈 B / 10개 능력치 전부 경기 효과)

투수 `mental`·`stamina` 능력치가 OVR 집계·컨디션 회복에만 쓰이고 한 구 한 구(at-bat) 결과엔 무영향이던 문제 해결. 이제 10개 능력치 전부가 실제 경기 계산에 직접 작용한다.

| 영역 | 작업 |
|---|---|
| **feat(stamina 지구력)** | `simulateAtBat` 에 투수 인게임 피로 추가 — 등판 후 누적 타자수가 스태미나 기반 용량(`18 + (stamina-50)*0.18`: 50=18타자·100=27·150=36) 초과 시 구위(`stuffAvg`)·제구(`control`)를 초과분 비례 최대 −12 감쇄 → 지친 투수 피안타·볼넷↑. 강판 임계도 용량 연동(NPC `capacity+7`, 메인 `capacity+12`) — **스태미나 50에서 기존 NPC25·메인30과 일치(하위호환)**, 스태미나만 추가로 스케일. |
| **feat(mental 클러치)** | 고압 상황도 `leverage`(후반 7회+ × 점수 접전 × 득점권 주자, 0~1) 산출 후 `simulateAtBat` 에 전달. 고멘탈 투수는 leverage 비례로 피안타·피홈런·볼넷 억제(침착), 저멘탈은 실점↑(멘탈붕괴). **평상시(leverage=0)엔 무영향** → 일반 경기 밸런스 불변, 접전·결승에서만 발현. |
| **verify** | 임시 통계 probe(20만 타석×케이스)로 효과 방향 확인 후 제거: leverage1 에서 멘탈 130 vs 30 피안타 17.0% vs 21.5%·피홈런 0.16% vs 1.02%, leverage0 무영향(20.7%≈20.6%); 지친 PA30 저스태미나 피안타 23.2%(생생 20.5%) vs 고스태미나 20.5%. `probe.mjs`·`probe-career.mjs` 회귀 통과. |

### v0.7.4 추가 ✓ — 결승 POV / 캐릭터 일러스트 16장 통합 + CSS 리깅 모션

캐릭터 생성 미리보기와 결승 POV 씬을 전용 일러스트로 교체. 에셋 없으면 기존 SVG 폴백(무손상).

| 영역 | 작업 |
|---|---|
| **에셋 16장** | 캐릭터 미리보기 12장(얼굴 6종×타자/투수, 420×562 WebP) + 결승 POV 4장(배경 2 landscape + 전경 누끼 2 투명 WebP). `manifest.js` 키 `charBatF1..F6`/`charPitchF1..F6`/`povBgBat`·`povBgPitch`/`povFgBat`·`povFgPitch`. |
| **캐릭터 미리보기** | `menu.js renderPreview` 가 `createImage("charBat"+faceId)`(폴백=기존 `createCharacterSVG`). 좌타(left/mixed)는 img 만 `scaleX(-1)`, idle 미세 흔들림(`nir-idle`). |
| **결승 POV wiring** | `finalAnim.js createPOVScene` 를 컨테이너(배경img z0 < SVG오버레이 z1 < 전경img z2) 구조로 재작성. 이미지 로드 성공 시 SVG 폴백 그룹(`bgGroup`/상대/뒤 실루엣) 숨김, 실패 시 기존 SVG 그대로. 손방향 `scaleX(-1)`(타격=left/mixed, 투구=left/lefty_rb). |
| **모션(CSS 리깅, 추가 이미지 0)** | 공 궤적/폭죽/라벨은 기존 SVG 엔진 유지. 타격 전경 방망이는 손 기준 CSS 회전 스윙(`swingFgImage`), 투구 전경 팔은 릴리스 push(`throwPush`). 폴백 시 기존 SVG 방망이 rect 회전(`swingSvgBat`). |
| **createImage 확장** | `imgStyle`(img 전용 transform — flip 용, 폴백 노드 미적용) + `onload` 콜백(레이어 합성용) 추가. |
| **verify** | 임시 DOM 스텁 스모크 테스트(키 구성·imgStyle flip·폴백·onload·양 POV 모드 좌타/우투·playPitch 무에러) 후 제거. `probe.mjs` 회귀 통과. **시각 렌더(전경 위치·스윙 모양)는 브라우저 실기 확인 필요** — 샌드박스 브라우저 부재. |
| **fix(결승모달 미표시)** | POV 컨테이너의 자식이 전부 `position:absolute` 라 flex 부모(`fieldWrap`/`sceneWrap`, justify-center)에서 **너비 0 으로 붕괴 → 씬 미표시**. 컨테이너에 `width:100%` 명시로 해결. 전경 소품은 `width:62%`(컨테이너 200px 초과로 방망이 끝 잘림) → `height:94%; aspect-ratio:1/1` 로 변경해 세로 맞춤. |

### v0.7.5 추가 ✓ — POV 결과 효과음 + 캐릭터 미리보기 1스크린 복구

| 영역 | 작업 |
|---|---|
| **feat(효과음)** | `audio.js` 합성음 추가 — `pitch`(공투척 휙), `out`(둔탁 범타), `crowd`(홈런 함성 스웰). 결과별 매핑은 `finalAnim.js playResultSfx`: 타격음 3종(HR=`homerun`+`crowd` / 안타=`hit` / 범타=`out`) + 삼진=`strikeout`, 모든 투구 릴리스에 `pitch`. **POV `playPitchSequence` 안에서 투구·컨택 타이밍에 맞춰 재생 → 결승 모달(replay)에도 사운드 적용**(이전엔 live 경로만 소리, 결승 모달은 무음이던 것 해결). `weekly.js appendEventLog` 의 중복 sfx 제거. |
| **fix(1스크린)** | v0.7.4 에서 캐릭터 미리보기를 기존 SVG(높이 104px)보다 큰 이미지(~128px)로 교체해 생성 화면이 살짝 넘치던 것 → 미리보기 이미지를 **높이 96px 고정·width:auto**(폴백 SVG 도 80×96)로 줄여 1스크린 맞춤 복구. |

### v0.7.6 fix ✓ — POV 결과글자 가림 + 전경 과대 + 생성화면 맞춤

| 영역 | 작업 |
|---|---|
| **fix(POV 결과 가림)** | 전경 소품(z2)이 결과 라벨(SVG z1)을 가려 "뭐 했는지" 안 보이던 것 → 결과 라벨/폭죽을 **최상단 오버레이 SVG(z3)** 로 분리해 항상 위에. |
| **fix(전경 과대)** | 전경 소품 `height:94%` 라 화면 중앙(상대/공/글자)을 덮던 것 → **`height:50%`, bottom 정렬**로 1인칭 하단 소품답게 축소. (POV 는 1인칭이라 **내 얼굴은 원래 안 나옴** — 방망이/손·던지는 팔만.) |
| **fix(생성화면 1스크린)** | 미리보기 이미지 96px 도 살짝 넘쳐 → **height 70px** + 카드 패딩 5→3px 로 더 축소. |

### v0.7.7 fix ✓ — 앱 셸(헤더/푸터 고정) + 푸터 게임명 + 17학년 오표기

| 영역 | 작업 |
|---|---|
| **앱 셸 스크롤** | `#app` `height:100dvh; overflow:hidden`, `.topbar`/`.bottombar` `flex-shrink:0`, `#view-root` `flex:1; min-height:0; overflow-y:auto` → **헤더/푸터 고정 + 본문만 스크롤**, 내용이 헤더/푸터 뒤로 안 가림. 스크롤/맨위로/뷰전환 리셋 대상 window→`#view-root`. |
| **푸터/헤더** | 푸터 버전 제거 → **게임명 오른쪽 정렬**(`.footer-title`), 상태메시지 `flex:1`(길게·ellipsis). 헤더 게임로고(`h1.logo`) 제거. |
| **fix(17학년)** | 프로/MLB 시즌시작 로그 `{grade}학년`(17학년) → `log.seasonStartPro`("만{age}세 시즌 시작")로 분기. 고교/대학만 학년. |

### v0.7.8 추가 ✓ — 훈련/UI 일괄 (밸런스·확인·MLB한글·셸·훈련방향안내)

| 영역 | 작업 |
|---|---|
| **밸런스 프리셋** | `two_way` `equalize:true` — 목표를 비중×cap 대신 전 스탯 공통값(최저 cap)으로 → 모든 능력치 같은 값 수렴. |
| **이적/계약 확인** | FA 잔류/이적·트레이드·MLB 오퍼 수락 직전 `confirm()` 한 번 더. |
| **MLB 한글명** | MLB 30구단 ko 로케일 한글 표기(뉴욕 양키스 등). |
| **앱 셸** | 헤더/푸터 고정 + 본문만 스크롤, 푸터 게임명 우측·버전 제거, 17학년→만N세. |
| **훈련방향 안내** | 자동훈련 최고비중 스탯이 cap 도달(밸런스는 전 스탯 목표 도달) 시 **일시정지 + 모달**(`trainSwitch`). "바꾸기"→일시정지 유지한 채 훈련방향 탭(carousel 0) 이동, 방향당 1회(`trainSwitchAcked`). |

### v0.7.9 추가 ✓ — KBO→MLB 포스팅(7년)/해외FA(9년)

KBO에서 MLB로 가는 경로가 없던 것 → 실제 규정 모델로 추가.

| 영역 | 작업 |
|---|---|
| **서비스 연수** | `player.kboSeasons` — `advanceToNextSeason` 에서 KBO(pro1/pro2) 시즌마다 누적. |
...
### v0.7.16 fix ✓ — 익명 상태 클라우드 저장 차단 (기기 간 동기화 함정 제거)

| 영역 | 작업 |
|---|---|
| **익명 클라우드 저장 차단** (`menu.js` + `weekly.js`) | 클라우드 세이브는 `saves/{uid}` 단일 문서인데 익명 로그인은 기기마다 uid 가 달라 다른 기기에서 불러올 수 없음(저장됐다고 믿는 함정). 저장 진입점 3곳(시작화면 패널·시즌종료·은퇴) 모두 **Google 로그인(비익명)일 때만** 허용. 시작화면은 익명 시 ☁️ 저장 비활성 + "Google 로그인 후 동기화" 안내. i18n `cloud.signInToSave`/`signInHint`(ko/en). 불러오기는 유지(같은 기기 복구·없으면 안내). |
...
### v0.7.23 추가 ✓ — 재능 타입에 따른 비시즌 훈련 특화 및 캐릭터 생성 재능 선택 UI 개선

| 영역 | 작업 |
|---|---|
| **재능별 비시즌 훈련** (`src/systems/offseason.js`) | 플레이어의 재능이 타자 중심이면 특훈/전지훈련/일반훈련에서 타자 능력치 위주로 상승하고, 투수 중심이면 투수 능력치 위주로 상승하도록 로직 전면 개편. 밸런스형 재능은 투타 균형있게 상승하는 기존 정책 유지. |
| **비시즌 이벤트 풀 필터링** (`src/systems/offseason.js`) | 플레이어의 재능 타입에 따라 타자 전용 이벤트와 투수 전용 이벤트를 구분하여 풀(pool)에서 걸러내어 선택되도록 개선. 또한 범용 이벤트의 효과(great/ok/bad)도 플레이어의 재능 방향성에 맞춰 자동으로 타자/투수 스탯 증가/감소로 맞춤 분기 처리. |
| **재능 선택 UI 디자인** (`src/views/menu.js`) | 캐릭터 생성 창에서 기존의 밋밋한 브라우저 네이티브 `<select>` 드롭다운을 제거하고, 클릭 시 고유 컬러 테마와 스탯 보정 상세가 카드 형태로 보여지는 고급스러운 **모달 팝업 재능 선택기**로 개편. |
| **시즌 중 콜업 일정/날짜 보존** (`src/systems/career.js`) | 시즌 중 콜업 시 달력 날짜(월/일)와 주차(`weekIndex`)가 시즌 처음(4월 1일)으로 초기화되던 버그 수정. 현재 날짜와 진행한 주차를 그대로 보존하며, 새 리그에서는 지나간 주차의 다른 팀 경기 결과들만 순회하며 백그라운드로 자동 시뮬레이션하여 자연스러운 순위/기록 흐름을 이어가도록 전면 수정. |
| **MLB 오퍼 나이 제한 추가** (`src/systems/career.js`) | 현실적인 이적 동향을 반영하여 만 32세 초과(33세 이상)인 고연령 시점에는 메이저리그(MLB) 구단으로부터 신규 오퍼(포스팅 및 FA 진출 기회)가 오지 않도록 오퍼 연령 상한선 제한 적용. |
| **노화 하락 스탯 로그 추가** (`src/systems/player.js` / `src/i18n/*`) | 노화로 인한 능력치 감소 발생 시, 어떤 능력치가 얼마나 감소했는지 시즌 개시 시점에 로그 텍스트로 사용자에게 투명하게 보여주도록 노화 상세 로깅 기능 추가. |
| **노화 스탯 하락 주간 분산** (`src/systems/week.js` / `src/systems/player.js` / `src/i18n/*`) | 시즌 종료 시 한 번에 대폭 하락하던 에이징 커브 하락 방식을 주간 단위로 잘게 분산하여 매주 미세하게 감쇄하는 방식으로 개편 (연간 하락 기댓값은 기존과 완전히 동일하게 보존). 하락 발생 시 주간 로그에 표시. |

### v0.7.22 fix ✓ — 광고 트리거 중앙화 (진로 선택 누락 해결) + GameMonetize 게임 ID 등록
...
- **능력치 스케일업 밸런스 (v0.7.19~):** NPC 캡 2배(MLB 400)·주인공 캡 NPC+50. 시뮬은 `statScale(stage)` 로 stamina/mental 절대기준만 정규화(매치업 raw → NPC 실제 강화), OVR 임계는 정규화 안 함. `probe-career` 는 전지훈련(휴식기 camp)을 안 거쳐 주인공을 과소평가하므로 **자동 AVG 검증 부정확** → 발달 전 선수 출장감·MLB 경기감은 **실기 확인**. (`handoff.md` 실기대기 2)
- **광고는 GameMonetize SDK (placeholder):** `index.html` `SDK_OPTIONS.gameId` 미설정 시 광고 미노출. 등록·승인 + gameId 입력 후 노출. SDK 메서드명은 가입 후 1회 확인. (`MONETIZATION.md`)
- **기본 언어 영어** (`DEFAULT_LOCALE="en"`). 영어 모드는 아시안게임·군 복무 제외, KBO→"Domestic League", 국내팀 닉네임만. 한국어는 토글 시 KBO·군대·아시안게임 그대로.
- **클라우드 동기화는 같은 Google 계정 필수** — 익명(미로그인)은 저장 차단(기기마다 uid 달라 동기화 불가).
...
- [ ] **멘탈/스태미나(능력치) 실질화** — 현재 at-bat 무영향. 멘탈=클러치(결승/끝내기/고리프) 보정, 스태미나(능력치)=투수 지구력/강판 임계 연동 등. 설계 3안 `handoff.md` (이슈 B)
- [ ] **BGM 안드로이드 백그라운드 pause** — `visibilitychange`/`pagehide` 시 정지 (`handoff.md` 이슈 A)
