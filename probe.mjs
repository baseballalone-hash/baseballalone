...
const { getBatterOVR, getPitcherOVR } = await import("./src/systems/ovrHelper.js");
...
function makePlayer(overrides = {}) {
  return {
    ...
    batter:  { contact: 70, power: 60, eye: 65, speed: 55 },
    pitcher: { velocity: 70, control: 65, breaking: 60, stamina: 70 },
...
const oppOvrs = oppNpc.roster.map(p => {
  if (p.role === "batter") return getBatterOVR(p);
  return getPitcherOVR(p);
});
...
state.player = makePlayer({
  ...
  batter:  { contact: 210, power: 180, eye: 195, speed: 160 },
  pitcher: { velocity: 215, control: 200, breaking: 185, stamina: 195 },
});
...
if (psFake) {
  const oppRosterAvg = psFake.opponent.roster.map(p => {
    if (p.role === "batter") return getBatterOVR(p);
    return getPitcherOVR(p);
  }).reduce((a,b)=>a+b,0) / psFake.opponent.roster.length;
...
state.player = makePlayer({
  ...
  teamName: mlbTeam,
  batter:  { contact: 250, power: 230, eye: 240, speed: 220 },
  pitcher: { velocity: 260, control: 250, breaking: 240, stamina: 250 },
});
