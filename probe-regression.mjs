...
const { capBonusForStat } = await import("./src/data/shopCatalog.js");
...
ok(m1.permanentPurchases.statCaps.contact === undefined, "statCaps.contact undefined");
...
ok(r.runs === 1, "runs 1");
ok(r.balance === 920 && r.totalEarned === 920, `balance/totalEarned 920 (got ${r.balance}/${r.totalEarned})`);
recordRun(180);
ok(state.regression.runs === 2, "runs 2 누적");
ok(state.regression.balance === 1100 && state.regression.totalEarned === 1100, "balance/totalEarned 1100 누적");
...
// capBoost contact (150점)
const c1 = purchasePermanent("capBoost", { stat: "contact" });
ok(c1.ok && c1.cost === 150 && c1.add === 5, `capBoost contact cost=${c1.cost} add=${c1.add}`);
ok(state.regression.balance === 850, `잔액 850 (got ${state.regression.balance})`);
ok(state.regression.permanentPurchases.statCaps.contact === 1, "statCaps.contact 1");

// capBoost contact 2회차 (200점)
const c2 = purchasePermanent("capBoost", { stat: "contact" });
ok(c2.ok && c2.cost === 200 && c2.add === 5, `capBoost contact cost=${c2.cost} add=${c2.add}`);
ok(state.regression.balance === 650, "잔액 650");
ok(state.regression.permanentPurchases.statCaps.contact === 2, "statCaps.contact 2");

// capBoost velocity (150점)
const c3 = purchasePermanent("capBoost", { stat: "velocity" });
ok(c3.ok && c3.cost === 150, `capBoost velocity cost=${c3.cost}`);
ok(state.regression.balance === 500, "잔액 500");

// capBoost velocity 2회차 (200점)
const c4 = purchasePermanent("capBoost", { stat: "velocity" });
ok(c4.ok && c4.cost === 200, `capBoost velocity 2회차 cost=${c4.cost}`);
ok(state.regression.balance === 300, "잔액 300");

// capBoost velocity 3회차 (250점)
const c5 = purchasePermanent("capBoost", { stat: "velocity" });
ok(c5.ok && c5.cost === 250, `capBoost velocity 3회차 cost=${c5.cost}`);
ok(state.regression.balance === 50, "잔액 50");

// 잔액 부족 (4회차 = 300점 필요)
const c6 = purchasePermanent("capBoost", { stat: "velocity" });
ok(!c6.ok && c6.reason === "insufficient_balance", `잔액 부족 (reason=${c6.reason})`);

// 잘못된 stat
const cBad = purchasePermanent("capBoost", { stat: "japan" });
ok(!cBad.ok && cBad.reason === "invalid_stat", `invalid_stat (reason=${cBad.reason})`);

// capBonusForStat 적용 검증
const contactBonus = capBonusForStat("contact", state.regression.permanentPurchases.statCaps);
const velocityBonus = capBonusForStat("velocity", state.regression.permanentPurchases.statCaps);
const powerBonus = capBonusForStat("power", state.regression.permanentPurchases.statCaps);
ok(contactBonus === 10, `contact 캡 보너스: ${contactBonus} (=10 기대)`);
ok(velocityBonus === 15, `velocity 캡 보너스: ${velocityBonus} (=15 기대)`);
ok(powerBonus === 0, `power 캡 보너스: ${powerBonus} (=0 기대)`);
...
resetRegressionMeta();
addBalance(99999);
purchasePermanent("capBoost", { stat: "contact" });   // +5 (cost 150)
purchasePermanent("capBoost", { stat: "contact" });   // +10 (cost 200)
purchasePermanent("capBoost", { stat: "contact" });   // +15 (cost 250)
const contactBonusAccum = capBonusForStat("contact", state.regression.permanentPurchases.statCaps);
ok(contactBonusAccum === 15, `contact cap 보너스: +${contactBonusAccum} (=15 기대)`);
const powerBonusAccum = capBonusForStat("power", state.regression.permanentPurchases.statCaps);
ok(powerBonusAccum === 0, `power cap 보너스: +${powerBonusAccum} (=0 기대)`);
...
addBalance(99999);
purchasePermanent("capBoost", { stat: "contact" });   // +5
purchasePermanent("capBoost", { stat: "contact" });   // +10
purchasePermanent("capBoost", { stat: "contact" });   // +15
// contact +15 보너스. 고교 기본 150 → contact 캡 165
const player = createPlayer({ name: "cap-test", talent: "contact" });
const capContactHigh = getPlayerStatCap(player, "contact");
const capPowerHigh = getPlayerStatCap(player, "power");
ok(capContactHigh === 165, `getPlayerStatCap(stage=high, stat=contact) = ${capContactHigh} (expected 165)`);
ok(capPowerHigh === 150, `getPlayerStatCap(stage=high, stat=power) = ${capPowerHigh} (expected 150)`);
player.stage = "univ";
ok(getPlayerStatCap(player, "contact") === 171 + 15, `univ contact cap = ${getPlayerStatCap(player, "contact")} (expected 186)`);
player.stage = "pro1";
ok(getPlayerStatCap(player, "contact") === 306 + 15, `pro1 contact cap = ${getPlayerStatCap(player, "contact")} (expected 321)`);
...
const baseSum = sumBaselineBatter.contact + sumBaselineBatter.power + sumBaselineBatter.eye + sumBaselineBatter.speed;
const focusSum = p2.batter.contact + p2.batter.power + p2.batter.eye + p2.batter.speed;
// battingFocus 는 4종 각 +10 = +40. rndInt(-4,4) 노이즈 양쪽 ±16 가능 → diff > 15 면 합격.
ok(focusSum - baseSum > 15, `battingFocus 시작값 합계 +${focusSum - baseSum} (>15 기대, 4종 +10 = +40)`);
...
const multi = createPlayer({
  name: "multi",
  talent: "sprinter",
  talents: ["sprinter", "slugger"],
});
ok(multi.talents.length === 2, `multi.talents 2개 (${multi.talents})`);
ok(multi.talent === "sprinter", `multi.talent='sprinter' (호환 필드)`);
const boost = combinedTalentBoost(multi);
// contact: 1.3 (sprinter) × 1.3 (slugger) = 1.69
// power:   1.5 (slugger)
// speed:   1.3 (sprinter)
// eye:     1.2 (sprinter)
ok(Math.abs(boost.contact - 1.69) < 0.001, `boost.contact = ${boost.contact} (expected 1.69 = 1.3×1.3)`);
ok(Math.abs(boost.power - 1.5) < 0.001, `boost.power = ${boost.power} (expected 1.5)`);
...
ok(state.regression.loadout.traits.length === 1 && state.regression.loadout.traits[0] === "steel_mental", "consume 후 loadout.traits 유지");
ok(state.regression.loadout.relics.length === 1 && state.regression.loadout.relics[0] === "lucky_bat", "consume 후 loadout.relics 유지");
...
// 직접 검증: prime_extend 캐릭터의 age 32→33 ageUp 결과 stat 감쇄 분포 (주간 미세 감쇄 버전)
const { declineStatsWeekly } = await import("./src/systems/player.js");
const trials = 200;
...
for (let i = 0; i < trials; i++) {
  ...
  for (let w = 0; w < 20; w++) declineStatsWeekly(a); // age 33 이나 delay 3 이므로 30+3=33 세 하한, weeklyChance 0.05
  if (a.batter.contact < before) primeDecays++;
  const b = createPlayer({ name: "b", talent: "contact" });
  b.age = 33; b.grade = 18;
  const before2 = b.batter.contact;
  for (let w = 0; w < 20; w++) declineStatsWeekly(b); // age 33 이므로 weeklyChance 0.15 로 감쇄 확률 더 높음
  if (b.batter.contact < before2) normalDecays++;
}
ok(primeDecays < normalDecays, `age 33 ageUp: prime decay ${primeDecays}/${trials} < normal ${normalDecays}/${trials} (prime_extend delay)`);

// 10.5 effectMultiplier — injuryRecoverySpeed ×2
const prosth = createPlayer({ name: "Pr", talent: "contact", relics: ["prosthetic"], relicLevels: { prosthetic: 3 } });
...
ok(effectMultiplier(multiBoost, "firstSeasonTrainBoost") === 2.6, `learner+past_life_notes firstSeasonTrainBoost = ${effectMultiplier(multiBoost, "firstSeasonTrainBoost")} (expected 2.6)`);
...
const draftPlayer = (relics, score) => {
  ...
  for (const s of ["contact","power","eye","speed"]) p.batter[s] = score;
  for (const s of ["velocity","control","breaking","stamina"]) p.pitcher[s] = score;
  p.fame = 0;
  return p;
};
// score 92 → 원래 round 2 (stage pro1), calling_card 보유 시 round 1 (stage pro1)
const d1 = draftPlayer([], 92);
const d2 = draftPlayer(["calling_card"], 92);
const r1 = kboDraft(d1);
const r2 = kboDraft(d2);
ok(r1.picked && r1.round === 2, `baseline score 92 → round ${r1.round} (expected 2)`);
ok(r2.picked && r2.round === 1, `calling_card score 92 → round ${r2.round} (expected 1)`);
// 1라운드 하한
...
// 11.4 mentor_letter — trainEfficiency ×1.2 (훈련 가중 곱셈)
// 직접 검증 어려움 — effectMultiplier 단위 검증만.
const mentor = createPlayer({ name: "Me", talent: "contact", relics: ["mentor_letter"] });
ok(effectMultiplier(mentor, "trainEfficiency") === 1.2,
   `mentor_letter trainEfficiency = ${effectMultiplier(mentor, "trainEfficiency")}`);
...
const luckyP = createPlayer({ name: "Lu", talent: "contact", relics: ["lucky_bat"], relicLevels: { lucky_bat: 2 } });
...
const combatP = createPlayer({ name: "Co", talent: "contact", traits: ["clutch"], relics: ["lucky_bat"], relicLevels: { lucky_bat: 2 } });
...
ok(effectMultiplier(ggP, "errorChance") === 0.7, `golden_glove errorChance multiplier = ${effectMultiplier(ggP, "errorChance")}`);
