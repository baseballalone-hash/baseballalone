...
state.player = {
  ...
  // 평균 이상 재능 — 드래프트 받을 정도
  batter:  { contact: 75, power: 65, eye: 70, speed: 60 },
  pitcher: { velocity: 78, control: 72, breaking: 65, stamina: 75 },
...
const seasonReports = [];
const peakStats = {
  batter:  { contact: 0, power: 0, eye: 0, speed: 0 },
  pitcher: { velocity: 0, control: 0, breaking: 0, stamina: 0 },
};
...
console.log(`            contact  power    eye    speed | velo  ctrl  break stam`);
console.log(`피크 stat : ${[pb.contact,pb.power,pb.eye,pb.speed].map(v=>String(v.toFixed(0)).padStart(7)).join("")} |${[pp.velocity,pp.control,pp.breaking,pp.stamina].map(v=>String(v.toFixed(0)).padStart(6)).join("")}`);
console.log(`최종 stat : ${[fb.contact,fb.power,fb.eye,fb.speed].map(v=>String(v.toFixed(0)).padStart(7)).join("")} |${[fp.velocity,fp.control,fp.breaking,fp.stamina].map(v=>String(v.toFixed(0)).padStart(6)).join("")}`);
const peakBatAvg = (pb.contact+pb.power+pb.eye+pb.speed)/4;
const peakPitAvg = (pp.velocity+pp.control+pp.breaking+pp.stamina)/4;
