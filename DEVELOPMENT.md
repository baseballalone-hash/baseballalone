...
### 5.4 현재 에셋 상태 (2026-06-01)
- **이미지 30장 가공·등록 완료** → `assets/img/*.webp`:
  - 이벤트/타이틀 14장 (title-hero + event-champion/hof/draft/intl/allstar/military/training/award/injury/retire/demote/fa/trade) — 모달 wiring 완료.
  - **캐릭터 미리보기 12장**(char-bat/pitch-f1..f6) — `menu.js renderPreview` wiring 완료.
  - **결승 POV 4장**(pov-bg-bat/pitch + pov-fg-bat/pitch 누끼) — `finalAnim.js createPOVScene` wiring + CSS 리깅 완료.
  - 에셋 없으면 전부 SVG/로고 폴백. **브라우저 실기 시각확인만 남음**(샌드박스 브라우저 부재).
- BGM 2곡(`assets/audio/bgm-{menu,game}.wav`) 완료. 효과음은 합성.
