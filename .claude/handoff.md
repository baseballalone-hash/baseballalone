...
> 마지막 갱신: 2026-06-02 (UX/밸런스/모네타이즈 대개편). 이 문서만 읽으면 이어서 진행 가능하도록 정리.
> **개발/테스트/에셋 절차는 `DEVELOPMENT.md`** 에 상세히 있음 (이 문서는 진행상황·결정 위주).

## 🔆 이번 세션(2026-06-02) 완료 — 상세는 README v0.7.13~v0.7.21
- **뒤로가기/모달 UX**: 안드로이드 뒤로가기 → 최상단 모달만 닫기(history 트랩, `main.js setupModalManager`). 모든 모달 닫기(×) 자동 주입. 군입대 모달만 `data-no-dismiss`. (v0.7.18)
- **시작화면**: 배경 이미지 풀스크린 + 하단 버튼 1스크린. 로그인 시 ☁️ 저장/불러오기. 클라우드 불러오기는 바로 게임 진입. (v0.7.13/18)
- **클라우드**: 익명(미로그인)은 저장 차단(기기 종속이라 동기화 불가) — Google 로그인 필수. 동기화는 양 기기 같은 Google 계정. (v0.7.16)
- **훈련 재설계**: 자동훈련이 "방향의 가장 부족한 능력치 1개"만 올림(훈련 종목과 분리). 밸런스=평균 재분배(총합 보존, 모두 같은 수치). 양방향(10종)은 5종 밸런스의 절반 속도. (v0.7.14)
- **통산/우승 리그별 분리(B안)**: 고교/대학/KBO1군/KBO2군/MLB메이저/MLB마이너. 시즌종료 수상 슬라이드에 리그별 우승·준우승. (v0.7.14)
- **국제대회 실제 브래킷 시뮬**: 8강→4강→결승(+동메달전) → 메달이 great/ok/bad·병역면제 결정(옛 20/70/10 난수 폐기). 실력 반영. (v0.7.15)
- **영어 버전 일반화**(en일 때): 아시안게임·군대 제거, KBO→"Domestic League", 국내팀 닉네임만. + **영어 디폴트**(`DEFAULT_LOCALE="en"`). (v0.7.17/19)
- **능력치 스케일업**: NPC 캡 = oldCap²/100(MLB 2배), 주인공 캡 = NPC+50. 시뮬은 `statScale(stage)` 로 **stamina/mental 절대기준만 정규화**(매치업 raw → NPC 실제 강함). OVR 임계는 정규화 안 함(주인공 ~옛 스케일). 영입 카드 "팀 평균 능력치" 표시. (v0.7.19)
- **모네타이즈**: 광고 = **GameMonetize.com SDK**(웹게임 전용). 시작/3시즌마다/은퇴 시 `showInterstitialAd()`=`window.sdk.showBanner()`. 광고 중 BGM pause. `MONETIZATION.md` 참고. (v0.7.20/21)

### ★ 실기/외부 확인 대기 (코드/probe 로 확정 불가)
1. **GameMonetize**: `index.html` `SDK_OPTIONS.gameId` 에 등록·승인 후 받은 ID 입력. SDK 메서드명(`showBanner` 전면)·초기화는 가입 후 그쪽 문서로 1회 확인(다르면 `ads.js` 호출부 한 줄 수정).
2. **능력치 스케일 밸런스**: probe 는 전지훈련(휴식기 camp)을 안 거쳐 주인공을 과소평가 → 자동 AVG 검증 부정확. 실기로 (a) 발달 전 선수 출장감(강해진 NPC 로테이션에 밀리는지), (b) MLB 경기 결과 자연스러운지 확인. 이상하면 능력치/캡 말고 **시뮬 계수**(softDiff·kChance 등) 재튜닝.
3. **모달 뒤로가기/×**: 안드로이드 실기에서 FA 오퍼 등 모달 뒤로가기→이어하기/새게임 정상, ×닫기, 군입대 닫기금지 확인.
4. **클라우드 동기화**: 두 기기 같은 Google 로그인 → A저장/B불러오기.
5. **teamFame 계수(×4)**: 명성 게이트가 너무 빡세면(무명이 오퍼 0) 조정.

## 🔆 이번 세션(2026-06-01 3차) 완료 — 상세는 README v0.7.11
- **실제 경기수**: 프로계 26주×3=78 동일 → 리그별 실측. MLB 162/AAA 150/AA 138/A 132/KBO 144/퓨처스 100, 고교 52/대학 44.
  (고교·대학 weeksPerSeason 은 토너먼트 결승 캘린더 고정, gamesPerWeek 만 조정.)
- **등판 리그 컷오프**: 투구 OVR < 팀 SP평균 50% 면 등판 0 (타자 올인이 메이저에서 투수로 끌려나가던 문제). 선발·구원·UI 연동.
- **트레이드 자동전송**(명성<50), **FA 오퍼 리미트 해제**(점수 높으면 전 구단), **커리어하이 리그 표시+가중치**.
- **타자/투수 밸런스 프리셋**, **멘토의 편지→훈련효율 ×1.2**, **최초 유물 2개 자동장착**, **재능 배수 조정**(파워·주루→컨택, 수비→주력).
- **fix**: 토스트 시즌종료 화면 미표시(drainPendingToasts 추가).
- **★ 실기(브라우저) 확인 대기 2건** (DOM/런타임 의존 — 코드/probe 로는 확정 불가):
  1. **첫 시즌 "마지막 주에서 멈춤"** — 결승모달을 모듈 플래그 대신 **DOM 존재(`[data-modal='final']`) 기반**으로 바꿔 self-heal. 실제로 멈춤 해소되는지 확인.
  2. **Google 로그인 redirect 실패** — `signInWithPopup`/`linkWithPopup` 우선+팝업차단 시 redirect 폴백으로 변경. 실기 로그인 확인 + Firebase 콘솔 **Authorized domains** 점검.
- **성능 주의**: MLB 주당 3→6경기라 주말 tick 시뮬 경기 ≈2배(30팀 ~90경기/주). 저사양 안드로이드에서 주말 넘김 버벅이면 gamesPerWeek 하향 검토.

## 🔆 이전 세션(2026-06-01 2차) 완료 — 상세는 README v0.7.2~v0.7.9
- 이슈 A(BGM 백그라운드 pause), 이슈 B(멘탈/스태미나 실질화)
- POV/캐릭터 일러스트 16장 통합 + CSS 리깅, POV 결과글자 가림·전경과대 수정, 효과음(타격3종+함성+공투척)
- 앱 셸 헤더/푸터 고정+본문 스크롤, 푸터 게임명, 17학년→만N세, 훈련 소수표시
- 밸런스 프리셋=전 스탯 동일값, 훈련방향 cap 도달 안내, 이적/계약 confirm, MLB 구단명 한글
- KBO→MLB 포스팅(7시즌)/해외FA(9시즌)
- **★ 실기(브라우저) 시각·동작 확인 대기:** ① 결승 POV 렌더/모션/효과음 ② 캐릭터 생성 1스크린 ③ 앱 셸 스크롤·푸터 ④ 훈련방향 cap 모달 ⑤ 이적/MLB 도전 모달 흐름 ⑥ BGM 백그라운드. (샌드박스 브라우저 부재로 코드/probe 검증만 완료.)
