...
> 마지막 갱신: 2026-06-01 (이슈 A — BGM 백그라운드 pause 수정 세션). 이 문서만 읽으면 이어서 진행 가능하도록 정리.
...
### 이슈 B — 멘탈만 다른 능력치보다 100+ 높음 + (더 깊은 문제) 멘탈·스태미나가 경기에 무영향
...
- **★ 더 근본 문제 (능력치 영향도 감사 결과):** 시뮬레이터 전수 조사 결과 **10개 능력치 중 8개만 실제 경기
  ...
  | **stamina(능력치)** | ❌ **직접 영향 없음** | 타석식 미사용·강판 PA30/6실점 고정(`shouldReplacePitcher`)·maxStamina 100 고정. 컨디션 회복+OVR집계만 |
  | **mental** | ❌ **직접 영향 없음** | 타석식 미사용. 컨디션 회복(`player.js:461`)+OVR집계만 |
...
| # | 항목 | 범위 | 상태 |
|---|---|---|---|
| 1 | **결승 POV / 캐릭터 생성 이미지 + 애니메이션** (아래 "★ 다음 개발 사항") | 중 | 미착수 (메인) |
| 2 | **이슈 A — BGM 안드로이드 백그라운드 pause** | 소 | ✅ 수정 완료 (실기 검증 대기) |
| 3 | **이슈 B — 멘탈/스태미나 능력치 실질화** (위 1·2·3 안 중 택1) | 소~대 | **설계결정 대기** |
...
3. **이슈 B (멘탈/스태미나 실질화)** — 사용자가 1/2/3 안 중 택1 하면 착수. 그 전까지는 보류.
